...
public class HuffmanTree<Key, E> {
    ...
    private void buildCodeTable() {
        ...
        codeTableHelper(root, "");
    }
    private void codeTableHelper(BinaryNodeKV node, String code){
        if(node.isLeaf())
            codeTable.put(node.getValue().toString(), code);
        else
        {
            codeTableHelper(node.getLeft(), code + "0");
            codeTableHelper(node.getRight(), code + "1");
        }
    }
    ...
    public String decode(String inCode) {
        ...
        while(inCode.length() > 0) {
            while (!currentNode.isLeaf()) {
                ...
                inCode = inCode.substring(1);
            }
            outStr += currentNode.getValue();
            currentNode = root; //Reset search to root for next letter
        }
        return outStr;
    }
    ...
    public static void main(String[] argv) {
        ...
        tree.codeTableHelper(tree.root, "");
    }
